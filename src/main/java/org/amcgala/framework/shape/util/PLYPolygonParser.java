...
public class PLYPolygonParser {
	 ...
	 * @throws IOException
	 */
	public static List<Polygon> parseAsPolygonList(String filePath, double scale) throws Exception {
	...
	* @throws IOException
	*/
	public static List<Polygon> parseAsPolygonList(InputStream inputStream, double scale) throws Exception {
	...
	for (int i = 0; i < vertexCount; i++) {
		vertices.add(new Vector3d(scanner.nextDouble() * scale, scanner.nextDouble() * scale, scanner.nextDouble() * scale));
		normals.add(new Vector3d(scanner.nextDouble() * scale, scanner.nextDouble() * scale, scanner.nextDouble() * scale));
		scanner.nextLine();
	}
	...
	for (int i = 0; i < faceCount; i++) {
		...
		if (currentFace == 4) {
			...
			polygons.add(new Polygon(vertices.get(index), vertices.get(scanner.nextInt()),
			vertices.get(scanner.nextInt()), vertices.get(scanner.nextInt()), normals.get(index)));
		} else if (currentFace == 3) {
			int index = scanner.nextInt();
			polygons.add(new Polygon(vertices.get(index), vertices.get(scanner.nextInt()), vertices.get(scanner.nextInt()), normals.get(index)));
		} else {
	...
	log.info("loaded " + vertexCount + " vertexes and " + faceCount + " faces");
	return polygons;
