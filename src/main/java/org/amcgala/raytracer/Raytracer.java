...
/**
 * Der RaytraceVisitor traversiert den {@link org.amcgala.scenegraph.SceneGraph} und berechnet die
...
public class Raytracer {
    ...
    public Raytracer() {
        ...
        viewPlane.setSampler(new RandomSampler(Framework.configuration.getInt("org.amcgala.raytracer.sampling")));
    ...
    public void traceScene() {
        for (int row = 0; row < viewPlane.getVerticalResolution(); row++) {
            for (int column = 0; column < viewPlane.getHorizontalResolution(); column++) {
                ...
                // FIXME Muss an DisplayList angepasst werden.
